...
**Use a memory calculator?** `8 GB / 100 MB ≈ 80 workers` is genuinely useful —
until you ask where the 100 MB came from, and what happens when the shop's workers
are 180 MB, the API's are 70 MB, one app starts shelling out to ImageMagick, and
traffic moves from one site to another overnight. A number frozen at deploy time
doesn't move with any of that.
